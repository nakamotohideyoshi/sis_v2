...
">
            ...
            <span><?php echo $information[0]['ItemName Long'];?></span><br />
            <label>Year :</label>
            <span><?php echo $information[0]['Year'];?></span><br />
            <?php  if (isset($this->session->userdata['web_response']) && array_key_exists('ShowLoanAmount', $this->session->userdata['web_response']) && $this->session->userdata['web_response']['ShowLoanAmount']=='Yes' && $information[0]['DollarAmount'] != null) {
            ...
            <?php } ?>
            <span><?php echo $information[0]['ItemStatus Long'];?></span><br />
    ...
    <div class="dropdown1">
        ...
        if(array_key_exists('ExtraInputBoxNeeded', $this->session->userdata['web_response']) && $this->session->userdata['web_response']['ExtraInputBoxNeeded'] =='Yes'){
            if($this->session->userdata['web_response']['ShowLoanAmount']=='Yes'){
                if ($this->session->userdata['web_response']['ItemName'] == 'AR Challenge'){
                            
            
					?>
                    <label> <?php echo array_key_exists('ResponseFromQuestionPicked', $this->session->userdata['web_response']) ? $this->session->userdata['web_response']['ResponseFromQuestionPicked'].'<br><br>' : '<br><br>'; ?></label>
                    <?php }
                    else{
                        ?>
                    

					<label>	<?php echo ($this->session->userdata['web_response']['ResponseFromQuestionPicked']).' Amount value must less than $'.$information[0]['DollarAmount'].'<br><br>'; ?></label>
                    <?php } } ?>                  

					<input type="text" name="amount" id="amount" placeholder="$$$"/><br/><br />
                    <input type="hidden" name="max-dollar-amount" value="<?= $information[0]['DollarAmount'] ?>" />
				<?php } 
				else { ?>
                    <input type="hidden" name="max-dollar-amount" value="<?= $information[0]['DollarAmount'] ?>" />
					<label>	<?php echo array_key_exists('ResponseFromQuestionPicked', $this->session->userdata['web_response']) ? $this->session->userdata['web_response']['ResponseFromQuestionPicked'].'<br><br>' : '<br><br>'; ?></label>
